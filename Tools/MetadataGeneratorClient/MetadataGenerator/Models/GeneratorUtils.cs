			...
			foreach (var op in operations)
			{
				...
				switch (operationTypeName)
				{
					case "functions":
						...
						{
							br.WriteLine(string.Format("public async Task<QueryResult<{0}>> {1}({2}, QueryObject queryObject = null) {{ var qr = await this.GetEntitiesAsync(\"{1}\", new Dictionary<string, object>() {{ {3} }}, queryObject); return new QueryResult<{0}> {{Rows = qr.Rows.Select(it => new {0}(it) ), TotalRows = qr.TotalRows }}; }}", op.ReturnType.Type, op.Name, parametersAsList, parametersAsDictionary));
						}
						else if (op.ReturnType != null && op.ReturnType.IsEntity && !op.ReturnType.IsCollection)
						{
							br.WriteLine(string.Format("public async Task<{0}> {1}({2}) {{ var it = await this.GetSingleEntityAsync(\"{1}\", new Dictionary<string, object>() {{ {3} }}); return new {0}(it); }}", op.ReturnType.Type, op.Name, parametersAsList, parametersAsDictionary));
						}
						else
						{
					...
					case "actions":
						...
						{
							br.WriteLine(string.Format("public async Task<IEnumerable<{0}>> {1}({2}) {{ var items = await this.GetEntitiesPostOperationAsync(\"{1}\", new Dictionary<string, object>() {{ {3} }}); return items.Select(it => new {0}(it)); }}", op.ReturnType.Type, op.Name, parametersAsList, parametersAsDictionary));
						}
						else if (op.ReturnType != null && op.ReturnType.IsEntity && !op.ReturnType.IsCollection)
						{
							br.WriteLine(string.Format("public async Task<{0}> {1}({2}) {{ var it = await this.GetEntityPostOperationAsync(\"{1}\", new Dictionary<string, object>() {{ {3} }}); return new {0}(it); }}", op.ReturnType, op.Name, parametersAsList, parametersAsDictionary));
						}
						else
						{
		...
		*/
		...
		public static void WriteNavigationProperties(BlockWriter br, string entityTypeName, Dictionary<string, NavigationProperty> etnp)
		{
			foreach (var navigationProperty in etnp)
			{
				...
				{
					br.WriteLine(string.Format("public {0} {1} {{ get {{ return new {3}(this.entity.Navigate{2}(\"{4}\", \"{1}\")); }} }}", returnType, navigationProperty.Key, navigationType, anp.EntityTypeName, entityTypeName));
				}
			}
			br.WriteLine();
		}
