...
namespace Client.Models.Utils.DAL.Common
{
        ...
        where T : class, IEntity
    {
        ...
        private bool HaveSameKeysLocal(Dto localEntity, Dto remoteEntity)
        {
            ...
            {
                if (Convert.ToInt64(localEntity[this.key[i]]) != Convert.ToInt64(remoteEntity[this.key[i]]))
                {
                    return false;
                }
            }
        ...
        private bool HaveSameKeysNavigation(Dto localEntity, string[] keyLocal, Dto remoteEntity, string[] keyRemote)
        {
            ...
            {
                if (Convert.ToInt64(localEntity[keyLocal[i]]) != Convert.ToInt64(remoteEntity[keyRemote[i]]))
                {
                    return false;
                }
            }
